...
class ChirpsAnomalyRecipe(BaseRecipe):
    ...
    def read_value(self, asset):
        ...
        return self._read_2d(asset, BucketType.STAGING)

    def read_normal(self, asset):
        """The published normal (Published assets-tier COG) as a 2D array."""
        from georiva.core.storage import BucketType
        return self._read_2d(asset, BucketType.ASSETS)

    @staticmethod
    def _read_2d(asset, bucket_type):
        import tempfile
        ...
        with tempfile.NamedTemporaryFile(suffix=".tif") as fh:
            fh.write(data)
            fh.flush()
            return rioxarray.open_rasterio(fh.name).squeeze(drop=True)
    ...
    def _output_variable(self, unit, resolved):
        ...
        if src is None:
            raise ValueError(
                "ChirpsAnomalyRecipe: source asset has no Variable to mirror"
            )
        si = resolved["value"].items[0]
        collection = self._published_collection(unit, si.collection.catalog)
